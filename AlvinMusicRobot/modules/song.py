...
def song(client, message):
    ...
    try:
        ...
        rep = "**🎵 Di upload oleh AlvinMusicRobot**"
...
async def ytmusic(client, message: Message):
    ...
    if not urlissed:
        await pablo.edit("Syntax Perintah Tidak Valid, Silakan Periksa Menu Bantuan Untuk Mengetahui Lebih Lanjut!")
        return
    ...
    await client.send_video(
        ...
        progress_args=(
            pablo,
            c_time,
            f"`Upload {urlissed} lagu dari YouTube Music!`",
            file_stark,
        ),
    )
    await pablo.delete()
